...
if __name__ == '__main__':
    ...
    print(np.unique(y_test_single, return_counts=True))
    # assert(False)
    ...
    def experiment(pre, limits, fname, long_title, unfrozen):
        ...
        for i in range(1, 6):
            ...
            for limit in limits:
                ...
                if (pre):
                    ...
                    new_model = astromcad.build_model(latent_size=100, ntimesteps=bts.X_train.shape[1], num_classes=bts.y_train.shape[1], contextual=0, n_features=bts.X_train.shape[2])
                
                val = limit//10
                train = limit - val
                astromcad.train(new_model, bts.X_train[:train], bts.y_train[:train], bts.X_val[:val], bts.y_val[:val], None, epochs=40)
        ...
        print("BTS Direct Performance: ", bts_direct_perf)


    limits = [100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1200, 1400, 1600, 1800, 2000, 2500, 3000, 3500, 4000]

    unfrozen = ['dense_3', 'latent', 'output']
    # experiment(pre=True, limits=limits, fname='bts_ft_last_two', long_title='BTS Fine Tuned, Output and Latnet', unfrozen = ['output', 'latent'])
    experiment(pre=True, limits=limits, fname='bts_ft_output', long_title='BTS Fine Tuned Only Output', unfrozen=['output'])
    # experiment(pre=True, limits=limits, fname='bts_ft_all', long_title='BTS Fine Tuned All', unfrozen=['gru1', 'gru2', 'dense_1', 'dense_3', 'latent', 'output'])
    experiment(pre=False, limits=limits, fname='bts_direct', long_title='BTS Direct Training', unfrozen=None)
